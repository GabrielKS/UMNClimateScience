...
# Note: Depending on how the input data are structured, choices must be made about whether to reduce the data using any() or all()
# (similarly, collapse_any_valid() or collapse_all_valid()) at each step. Users of the universal mask should be aware of which
# of these choices have been made.
import os
...
def main():
    # Get all the data and find valid values across time for each model run
    ...
    # Collapse the newly-added dimension by 'and'-ing or 'or'-ing all the values in each grid cell together
    # Also split the dataset into an array with one entry for each variable
    mask_by_variable = {variable: combined_dataset[variable].any(dim="modelrun")  # One instance where any() vs. all() must be contemplated
    ...
    resources.print_as_binary(collapsed_mask)
